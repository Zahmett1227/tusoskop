...
const ACCENT_CLASSES = {
  emerald: { dot: "bg-emerald-400", bar: "bg-emerald-500/40", badge: "bg-emerald-500/10 text-emerald-300 border-emerald-500/20" },
  rose:    { dot: "bg-rose-400",    bar: "bg-rose-500/40",    badge: "bg-rose-500/10 text-rose-300 border-rose-500/20"         },
  cyan:    { dot: "bg-cyan-400",    bar: "bg-cyan-500/40",    badge: "bg-cyan-500/10 text-cyan-300 border-cyan-500/20"         },
  slate:   { dot: "bg-slate-500",   bar: "bg-slate-600/40",   badge: "bg-slate-700/50 text-slate-400 border-slate-600/30"     },
};
  ...
  return (
    <div className="overflow-hidden rounded-3xl border border-slate-700/50 bg-gradient-to-b from-slate-900 to-slate-950 shadow-2xl shadow-black/40">
      ...
      <div className="relative overflow-hidden px-5 pt-5 pb-4 border-b border-slate-800/60">
        ...
        {/* Risk uyarısı — varsa header altına */}
        {riskInfo && (
      ...
      <div className="px-5 py-4 space-y-3">
        ...
        {!loading && isPremiumRequired && (
          <div className="rounded-2xl border border-violet-500/20 bg-gradient-to-br from-violet-500/8 to-cyan-500/5 px-5 py-6 text-center">
            <div className="mx-auto mb-3 flex h-12 w-12 items-center justify-center rounded-2xl bg-violet-500/15 border border-violet-500/25 text-2xl">
              ⭐
            </div>
            ...
            <p className="mt-1.5 text-xs leading-relaxed text-slate-400 max-w-xs mx-auto">
              AI destekli günlük plan, FSRS verilerin ve konu yeterlilik düzeyini
              analiz ederek kişiselleştirilmiş bir çalışma yolu oluşturur.
              Yalnızca premium üyelere sunulmaktadır.
            </p>
          </div>
        )}
          ...
          <>
            {plan.recommendation.dailyPlan.map((item, i) => (
              <PlanItem
                ...
                item={item}
                index={i}
                total={plan.recommendation.dailyPlan.length}
                theme={theme}
            ...
            {plan.status === "fallback" && (
              <p className="text-[10px] text-slate-700 text-right pr-1">
                AI erişilemedi · Akıllı varsayılan plan
              </p>
            )}
          </>
        )}
