...
### 13.6 K8 aktive + C3 Purchase→ViewContent geçişi + ilk veri (16 Temmuz 2026)

**K8 canlıya alındı (13.5'in devamı):** Deploy sonrası K8 kreatifi (`1069162925577450`) + reklamı C3'e eklendi. Kullanıcı düzeltmesiyle görsel **4,8 ★ App Store** oldu (5,0 değil; değerlendirme sayısı yazılmadı; per-yorum yıldızlar kaldırıldı — dürüstlük). K6 + K8 rotasyona girdi.

**C3 Purchase optimizasyonu boğuldu → ViewContent'e geçildi (kullanıcı: "C3'ü LPV'ye çevir").**
- **Sorun:** C3 (Purchase optimizasyonu, dar sıcak kitle) 2 günde ~₺13 harcadı, günde ~₺4 — Purchase 0 sinyalle Meta teslimatı boğuyordu. Purchase optimizasyonu 0 dönüşümle öğrenemiyor (plan İlke 2: hacimli olaya optimize et).
- **LPV reddedildi:** `LANDING_PAGE_VIEWS` + pixel-only promoted_object OUTCOME_SALES altında bu hesapta yine "Promoted Object Invalid" (log 12.4'teki aynı kısıt). Ayrıca **yayındaki ad set'in optimizasyonu düzenlenemez** ("Can't edit published ad set → yeni ad set oluştur").
- **Çözüm:** Yeni ad set **`52565748678163`** ("C3 · Sıcak kitle → ViewContent", OFFSITE_CONVERSIONS → **CONTENT_VIEW**, aynı sıcak kitle + advantage_audience off + CBO). İçine K6 (`52565748902563`) + K8 (`52565748939363`) eklendi, ikisi de ACTIVE + Meta onayından geçti. Eski Purchase ad set **`52564951353563` → PAUSED**. **Bonus:** geçiş frekansı sıfırladı (eski 2,79 → yeni 1,37).
- **MCP kesintisi notu:** Geçiş sırasında Meta MCP koptu + sonra onay kapısı birkaç tur çağrıları blokladı; ara ara "yapıldı" yazıp sonra doğrulayınca durum farklı çıktı. Ders: **canlı reklam hesabında her adımı `ads_get_ad_entities` ile DOĞRULA, anlatıya güvenme.** Nihai doğrulanmış durum yukarıdaki.

**İlk C3 + satış katmanı verisi (13-16 Tem, ~3 gün):**
- C3 toplam ~118 gösterim / ~₺20 (hâlâ dar kitlede düşük teslimat). Bugün 56 gösterim / ₺6,26 / freq 1,75.
- **K6/K8 kıyası:** örneklem çok küçük (onlarca gösterim), sonuç çıkarılamaz — birkaç gün daha.
- **Dönüşüm:** `EylulPaketiClick` düzenli fire ediyor (sonuç ekranı satış kartı çalışıyor ✅). Ama **InitiateCheckout = 0, web Purchase = 0** → yeni web satışı yok.
- **App Store IAP satışı:** 13 Tem ~21:59'da admin'de bir Apple IAP satışı (kullanıcı teyit etti). **Meta web pixel'i App Store IAP'ı GÖREMEZ** (İlke 1/§08 kör kuyu) — beklenen, bug değil. **ROI admin panelinden (web + App Store) ölçülmeli, Meta'nın Purchase'ıyla değil.**
- **C1/C2 sağlıklı:** C1 CTR ~%3,6 (₺80/gün, freq 1,08), C2 CTR ~%1,9 (₺34/gün). Genel huni akıyor.

**Plana göre:** ~3. gün, plandan ileride, sinyal merdiveni doğru. Web satış yokluğu beklenen (plan §05: düzenli satış 2. ayda). Darboğaz: küçük sıcak havuz + web-checkout sürtünmesi + iOS→App Store sızıntısı. **Açık en yüksek kaldıraç:** in-app tarayıcı satış deep-link düzeltmesi (anonim Instagram kullanıcısı `/app?intent=plus`'ta Google-giriş duvarına tosluyor; `EylulPaketiCard` hard-nav yerine `isInAppBrowser()` + "Tarayıcıda Aç" ipucu gerekiyor). Kod işi, henüz yapılmadı.
